...
export const useCreateServiceMethods = () => {
    ...
    useEffect(() => {
        setSelectedButton(service?.category)
        setDateFrom(service?.from)
        setDateUntil(service?.until)
        setCurrentTime(service?.hour)

        const ids = service?.employees?.map((element) => {
            return element._id;
        })
        setAssignedIdsEmployees(ids)
        setAssignedEmployees(service?.employees)
        setAssignedSupervisores(service.supervisor)
        setAssignedSupervisor(service.supervisor)
        setSelectedAddress(service?.address)
        setAssignedCustomer(service?.customer)
        setBussinessAdditional(service?.comments)
    }, [service])
    ...
    const createService = () => {
        ...
        (async () => {
            ...
            const response = await apiService.update(accessToken, data);
